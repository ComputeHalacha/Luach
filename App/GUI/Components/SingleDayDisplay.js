...
export default class SingleDayDisplay extends React.PureComponent {
  ...
  render() {
    const { appData, jdate, isToday } = this.props,
      ...
      todayText = isToday && (
        <Text style={styles.todayText}>{`TODAY${isDayOff ? '*' : ''}`}</Text>
      ),
      isSpecialDay =
      ...
      sunrise =
        suntimesMishor && suntimesMishor.sunrise
          ? Utils.getTimeString(suntimesMishor.sunrise, GLOBALS.IS_24_HOUR)
          : 'Sun does not rise',
      sunset =
        suntimes && suntimes.sunset
          ? Utils.getTimeString(suntimes.sunset, GLOBALS.IS_24_HOUR)
          : 'Sun does not set',
      candleLighting = jdate.hasCandleLighting() && (
        <Text style={styles.darkText}>
          {'Candle-lighting: ' +
            Utils.getTimeString(
              Zmanim.getCandleLightingFromSunTimes(suntimes, location),
              GLOBALS.IS_24_HOUR
            )}
        </Text>
      ),
        ...
        <View>
          ...
          <View style={styles.mainSectionView}>
            <Text style={styles.date}>
              <Text style={styles.dateHeb}>{jdate.toString()}</Text>
              <Text>{'\n'}</Text>
            ...
            {eiruvTavshilin}
            {!isYomTov && (
              <Text style={styles.darkText}>{`Sedra of the week: ${jdate
            ...
            <View style={styles.bottomSection}>
              <View style={{ flex: 0 }}>
                <TouchableWithoutFeedback onPress={this.showDateDetails}>
                  <View>
                    <Text style={styles.darkText}>{'Sunrise: ' + sunrise}</Text>
                    <Text style={styles.darkText}>{'Sunset: ' + sunset}</Text>
                    <Text
                      style={{
                        color: '#666',
                        fontSize: 10,
              ...
              >
                {flag && <FlagComponent onPress={this.showProblems} />}
                {appData.Settings.showEntryFlagOnHome && (
                  <View>
                    <DaysLastEntryComponent
                      ...
                      currDate={jdate}
                      onPress={() =>
                        this.navigator.navigate('Entries', { ...this.props })
                      }
                    />
                    <EntriesComponent list={entries} edit={this.editEntry} />
                    {isPossibleHefsekDay && (
        ...
        <View style={styles.menuView}>
          <TouchableWithoutFeedback onPress={this.newEntry} style={{ flex: 1 }}>
            <View style={styles.menuItemView}>
              <Icon color="#778" name="remove-red-eye" size={menuIconSize} />
          ...
          >
            <View style={styles.menuItemView}>
              <Icon color="#778" name="event" size={menuIconSize} />
          ...
          {appData.Settings.showEntryFlagOnHome && !hasHefsek && (
            ...
            >
              <View
                style={[
                  styles.menuItemView,
                  hasMikvah || hasShailah ? styles.lastMenuView : null,
                ]}
              >
                <Icon color="#778" name="flare" size={menuIconSize} />
          ...
          {appData.Settings.showEntryFlagOnHome && !hasShailah && (
            <TouchableWithoutFeedback
              onPress={() => this.toggleTaharaEvent(TaharaEventType.Shailah)}
              style={{ flex: 1 }}
            >
              <View
                style={[
              ...
              >
                <Icon color="#778" name="report-problem" size={menuIconSize} />
          ...
          {appData.Settings.showEntryFlagOnHome && !hasMikvah && (
            ...
            >
              <View style={[styles.menuItemView, styles.lastMenuView]}>
                <Icon color="#778" name="beenhere" size={menuIconSize} />
...
function DaysLastEntryComponent(props) {
  if (props.lastEntryDate) {
    ...
    if (dayNum > 1) {
      return (
        <TouchableOpacity onPress={props.onPress}>
          <View style={[styles.additionsViews, { flexDirection: 'row' }]}>
  ...
  if (dayOfSeven && dayOfSeven > 0 && (dayOfSeven < 8 || isYomKippurOrTBA)) {
    return (
      <View style={styles.additionsViews}>
        ...
        >
          {Utils.toSuffixed(dayOfSeven) + ' day of 7\nfrom Hefsek Tahara'}
        </Text>
        ...
        {props.list.map((te, i) => {
          ...
          return (
            <TouchableOpacity
              ...
              onLongPress={() => {
                if (te.taharaEventType === TaharaEventType.Hefsek) {
            ...
            >
              <View style={[styles.badgeView, { backgroundColor: bgColor }]}>
                ...
                />
                <Text style={styles.taharaEventsText}>{te.toTypeString()}</Text>
              </View>
            </TouchableOpacity>
          );
        })}
