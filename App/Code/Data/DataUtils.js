...
export default class DataUtils {
    ...
    static async SettingsFromDatabase() {
        ...
        try {
            ...
            settings = new Settings({
                ...
                fourDaysHefsek: !!dbSet.fourDaysHefsek,
    ...
    static async LocationFromDatabase(locationId) {
        ...
        try {
            const ls = await DataUtils._queryLocations('locationId=?', [
                locationId,
            ]);
            if (ls.length > 0) {
                location = ls[0];
        ...
        try {
            ...
            list = results.list.map(
                (k) =>
                    new Kavuah(
                        k.kavuahType,
                        entries.find((e) => e.entryId === k.settingEntryId),
                        k.specialNumber,
    ...
    static async DeleteKavuah(kavuah) {
        ...
        try {
            await DataUtils._executeSql(
                'DELETE from kavuahs where kavuahId=?',
                [kavuah.kavuahId]
            );
