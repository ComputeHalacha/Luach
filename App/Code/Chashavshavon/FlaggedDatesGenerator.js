...
export default class FlaggedDatesGenerator {
    ...
    _findEntryDependentKavuahProblemOnahs(entry) {
        ...
        for (let kavuah of this.kavuahs.filter(
            ...
            We assume that the haflagah of 10 was "instead" of the 11, and not an unrealted entry with the 11 still to come.
            ...
            const numEntriesSinceSetting = this.entries.filter(e =>
                e.abs >= settingEntry.abs && e.abs <= entry.abs).length;
        ...
        if (!this.settings.noProbsAfterEntry) {
            ...
            return !this.entries.some(
                en =>
                    en.abs >= jdate.Abs - 7 &&
                    (en.abs < jdate.Abs ||
                        (en.abs === jdate.Abs && en.nightDay < nightDay))
            );
        }
    }
}
