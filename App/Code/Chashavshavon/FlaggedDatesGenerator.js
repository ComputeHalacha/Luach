...
export default class FlaggedDatesGenerator {
    constructor(realEntrysList, settings, kavuahs) {
        this.realEntrysList = realEntrysList;
     ...
     * @param {Entry} [settingEntry] optional entry to pass on to the canAddFlaggedDate function.
     */
    addProblem(probFlag, settingEntry) {
        if (this.canAddFlaggedDate(probFlag, settingEntry)) {
            let probOnah = this.probOnahs.find(po => po.isSameProb(probFlag));
            if (!probOnah) {
